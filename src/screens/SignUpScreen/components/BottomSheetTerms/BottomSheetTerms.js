    ...
    return (
        <BottomSheet
            ...
            <View style={{ height: 500 }}>
                <Text style={styles.terms}>{props.title || 'Условия использования'}</Text>
                <Text
                    style={{
                        marginVertical: 20,
                        textAlign: 'justify',
...
export default function BottomSheetTermsComponent({ panelRef, onClose, title, text }) {
    return <BottomSheetTerms ref={panelRef} onClose={onClose} title={title} text={text} />;
}
